...
func (h controllerHandler) createHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Error(":: createHandler .. CreateController error ::", log.Fields{"Error": err})
...
func (h controllerHandler) putHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Error(":: putHandler .. CreateController error ::", log.Fields{"Error": err})
		http.Error(w, err.Error(), http.StatusConflict)
		return
	}
...
func (h controllerHandler) getHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(name) == 0 {
		...
		if err != nil {
			log.Error(":: getHandler .. GetControllers error ::", log.Fields{"Error": err})
		...
		if err != nil {
			log.Error(":: getHandler .. GetController error ::", log.Fields{"Error": err.Error()})
			if strings.Contains(err.Error(), "db Find error") {
				http.Error(w, err.Error(), http.StatusNotFound)
			} else {
				http.Error(w, err.Error(), http.StatusInternalServerError)
...
func (h controllerHandler) deleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Error(":: deleteHandler .. DeleteController error ::", log.Fields{"Error": err})
