...
var _ = Describe("SFCClientSelectorIntent", func() {
	...
	Describe("Create SFC client selector intent", func() {
		It("successful creation of sfc client selector intent", func() {
			...
			_, err = (*sfcClient).CreateSfcClientSelectorIntent(sfcClientSelectorIntent, "testproject", "ca", "v1", "dig", "netctl", "sfcIntentName", false)
			Expect(err).To(BeNil())
		})
		...
		It("Parent resource does not exist", func() {
			...
			// test SFC Client Selector intent creation, but give SFC Intent  name that does not exist
			_, err = (*sfcClient).CreateSfcClientSelectorIntent(sfcClientSelectorIntent, "testproject", "ca", "v1", "dig", "netctl", "sfcIntentNameXYZ", false)
		...
		It("successful creation of sfc client selector intent with update version of call", func() {
			...
			_, err = (*sfcClient).CreateSfcClientSelectorIntent(sfcClientSelectorIntent, "testproject", "ca", "v1", "dig", "netctl", "sfcIntentName", true)
			Expect(err).To(BeNil())
		})
		It("successful creation of sfc client selector intent with update version of call", func() {
			// set up prerequisites
	...
	Describe("Get all sfc client selector intents", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			_, err := (*sfcClient).GetAllSfcClientSelectorIntents("testproject", "ca", "v1", "dig", "netctl", "sfcIntentName")
		...
		It("Parent SFC Intent does exist - No SFC Client Selector Intents - should return empty list", func() {
			...
			list, err := (*sfcClient).GetAllSfcClientSelectorIntents("testproject", "ca", "v1", "dig", "netctl", "sfcIntentName")
			Expect(err).To(BeNil())
	...
	Describe("Get sfc client selector intent", func() {
		It("Parent SFC Intent does not exist - return not found error", func() {
			_, err := (*sfcClient).GetSfcClientSelectorIntent("sfcIntentName", "testproject", "ca", "v1", "dig", "netctl", "sfcIntentName")
			Expect(strings.Contains(err.Error(), "not found")).To(Equal(true))
		})
		It("Successful get of sfcClientSelectorIntent", func() {
			...
			_, err = (*sfcClient).GetSfcClientSelectorIntent("sfcClientSelectorIntentName", "testproject", "ca", "v1", "dig", "netctl", "sfcIntentName")
			Expect(err).To(BeNil())
		})
		It("should return error for general db error", func() {
			mdb.Err = pkgerrors.New("db Find error")
	...
	Describe("Delete SFC Client Selector intent", func() {
		It("successful delete", func() {
			...
			err = (*sfcClient).DeleteSfcClientSelectorIntent("sfcClientSelectorIntentName", "testproject", "ca", "v1", "dig", "netctl", "sfcIntentName")
			Expect(err).To(BeNil())
		})
