...
Life cycle management of composite applications is complex. Instantiation, updates and terminations of the composite application across multiple K8s clusters (Edges and Clouds), monitoring the status of the composite application deployment, Day 2 operations (modification of the deployment intent, upgrades, etc..) are a few of the complex operations.

The number of K8s clusters (Edges or clouds) could be in tens of thousands. The number of composite applications that need to be managed could be in the hundreds. The number of applications in a composite application could be in the tens.  The number of micro-services in each application of the composite application can be in the tens. Moreover, there can be multiple deployments of the same composite application for different purposes. To reduce the complexity, all of these operations are to be automated. There shall be one-click deployment of the composite applications and one simple dashboard to know the status of the composite application deployment at any time. Hence, there is a need for Multi-Edge and Multi-Cloud distributed application orchestrator.
...
|                        |                                                                                                                                  |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Cluster Provider       | The cluster provider is someone who owns clusters and registers them.                                                                    |
...
In this initial release of EMCO, a built-in generic placement controller is provided in the `orchestrator`.  HPA Placement Controller is an example of Placement controllers. Some action controllers provided with EMCO are the OVN Action, Traffic and Generic Action controllers.
...
This type of Logical Cloud provides most of the capabilities that an Admin Logical Cloud provides but at the user-level like a Standard Logical Cloud. New namespaces are created, with new user and kubeconfig files. However, the EMCO project can now request enhanced set of permissions/privileges, including targeting cluster-wide Kubernetes resources.
...
### Resource  (Rsync)
This microservice is the one which deploys the resources in edge/cloud clusters. 'AppContext' created by various microservices are used by this microservice. It takes care of retrying, in case the remote clusters are not reachable temporarily.
Orchestrator and other controllers like ncm and dcm communicate with rsync using gRPC.
...
- Rsync event : Tells what is the event, can be instantiate, terminate, update etc.
- UCID : Contains the unique appContextID
- Status : Tells if status pending or done

One main thread runs per AppContext and picks up next element from the AppContext queue in the “pending” state. If not running, main thread is started.
Status Flag used by orchestrator or other controllers to find out the current status of the AppContext. This will change as more events are processed.
If an event is not valid for the current state (example terminate before instantiate) that event is skipped, and status marked as error in the queue. Rsync will move on to next event. Every event is completed before picking up next event. (One exception terminate while waiting for cluster to be ready, see below )

Main thread for a AppContext starts a stop thread. Then the main thread picks next pending event from the queue, decides on whether to apply, delete or read resources based on the event. It starts one thread per App and per cluster to handle the operation required. Within a cluster resources are handled in order as specified in the AppContext. Dependency between applications and resources will be added later.

#### Instantiation
Instantiate event follows this basic flow as outlined above.

#### Termination
If terminate event is received while there are other events pending, only the current event being processed will be completed.
All other events will be skipped. For example, if the queue has items -> instantiate, update and rsync is processing instantiate and at that time terminate event is added, then instantiation will be completed but update will be skipped. If the current event is waiting for cluster ready at the time terminate is received (and terminate is valid state chnage), that wait is stopped, and that cluster will fail the current event (for example instantiation, update, etc).
...
Orchestrator creates a new AppContext with includes all the resources as desired after update/migrate. Rsync update gRPC is called with two AppContexts. The first is the Initial AppContext (which must be in Instantiated state). The second is the Update AppContext. The Update happens in two phases.
...
If it finds an active AppContextID there, it creates the AppContextData and starts the main thread for handling the pending AppContextIDs which were recorded as active contextIDs.
